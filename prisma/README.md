...
2. **User & Shop** — Multi-tenant: mỗi store có owner, roles gắn `shopId`, catalog gắn `shopId`.
...
7. **Staff** — `WorkingHour` (store), `StaffSchedule`, `StaffDayOff`.

Trong schema có **comment** giải thích các quyết định thiết kế (ví dụ timezone store, exclude constraint cho `StaffBooking` — có thể cần SQL thủ công sau migrate).
